...
namespace DevSecretStash.Cli.Tests;
...
public class CliTestFixture : WebApplicationFactory<Program>, IDisposable
{
    ...
    public CliTestFixture()
    {
        ...
        _connection.Open();
        _tempDir = Path.Combine(Path.GetTempPath(), $"devsecretstash-test-{Guid.NewGuid()}");
...
/// <summary>
/// Test-specific ConfigStore that uses a custom directory instead of ~/.devsecretstash
/// </summary>
internal class TestConfigStore : ConfigStore
{
    ...
    public TestConfigStore(string configDir, string serverUrl)
    {
        ...
        var config = new DevSecretStashConfig { ServerUrl = serverUrl.TrimEnd('/') };
        Directory.CreateDirectory(configDir);
        File.WriteAllText(_configPath, JsonSerializer.Serialize(config));
    }

    public override DevSecretStashConfig Load()
    {
        if (!File.Exists(_configPath))
            return new DevSecretStashConfig();
        return JsonSerializer.Deserialize<DevSecretStashConfig>(File.ReadAllText(_configPath)) ?? new DevSecretStashConfig();
    }
}

/// <summary>
/// Test-specific CredentialStore that uses a custom directory instead of ~/.devsecretstash
/// </summary>
